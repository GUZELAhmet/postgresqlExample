...
select * from meslekler order by maas desc offset 5 row fetch  next 3 row only;

select * from meslekler order by maas desc offset 2 limit 5;
...
select filmler.film_name, filmler.category, aktorler.actor_name 
from filmler LEFT JOIN aktorler
on filmler.film_id=aktorler.film_id;

-- SORU2: Tüm actor_name'leri ve film_name'lerini listeleyiniz.
SELECT actor_name,film_name
FROM filmler as C right JOIN aktorler as D
